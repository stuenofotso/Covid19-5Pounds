...
/** Main class */
object PredictMyCustomKaggleDataOld {
  ...
  def predictForFeatureAndTimeWindow(indexedDf:DataFrame, featureName:String, timeWindow:Int, keyColumnsNames:String*): DataFrame={
    ...
    val model = CrossValidatorModel.load(s"models/KaggleDataModel/GBTRegressionModel_total_cov_$featureName.ml")
    ...
    val filteredDf = indexedDf.where($"Date">=date_sub(lit(maxKnownDate), Math.max(HORIZON, PREDICTION_HORIZON)))
    ...
    predictions.values.reduce(_ union _)
  }



  @scala.annotation.tailrec
  def predict(H:Int, inputDf:DataFrame): DataFrame = {

    if(H>=PREDICTION_HORIZON) return inputDf

    println(s"starting batch H=$H")
    /*val inputDf =  spark.read.format("csv")
      .option("header","false")
      .option("inferSchema", "true")
      .load(s"kaggle_data/tmp/predictDf$parity")
...
*/
    ...
    predict(H+3, inputDf.union(predictedf))
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    groupedDfES.printSchema()

    val maxKnownDateES = new Date(groupedDfES.select($"Date").orderBy($"Date".desc).first().getAs[Timestamp]("Date").getTime)

    println(s"known dates : Kaggle=$maxKnownDateKaggle ; ES=$maxKnownDateES")


    val prevGroupedDf = groupedDfKaggle.union(groupedDfES)
 ...
 */
    ...
    val resUnfoundCountryKaggleDf = Seq("ConfirmedCases", "Fatalities").par.map(feature=>predictForFeatureAndTimeWindow( unfoundCountryKaggleDf.select("Country_Region", "Country_RegionIndex", "Province_State", "Province_StateIndex", "Date", feature), feature, maxKnownDateKaggle.toLocalDate.until(maxKnownDateES.toLocalDate, ChronoUnit.DAYS).toInt,  "Country_RegionIndex", "Province_StateIndex", "Date"))
    ...
    val predictedUnfoundCountryKaggleDf = resUnfoundCountryKaggleDf.head.join(resUnfoundCountryKaggleDf.last, Seq("Country_RegionIndex", "Province_StateIndex", "Date"))
      .select(resUnfoundCountryKaggleDf.head("Province_State"), resUnfoundCountryKaggleDf.head("Country_Region"), resUnfoundCountryKaggleDf.head("Date"), col("ConfirmedCases"), col("Fatalities"), resUnfoundCountryKaggleDf.head("Country_RegionIndex"), resUnfoundCountryKaggleDf.head("Province_StateIndex"))
      .cache()

    predictedUnfoundCountryKaggleDf.printSchema()

      val inputDf = prevIndexedDf.union(predictedUnfoundCountryKaggleDf)
        .select("Country_RegionIndex", "Province_StateIndex", "Date", "ConfirmedCases", "Fatalities")

        //.write.mode(SaveMode.Overwrite).format("csv").save("kaggle_data/tmp/predictDf0")




    val predictedDf =  predict(0, inputDf)
      .withColumn("ConfirmedCases", sum("ConfirmedCases").over(
      Window
        .partitionBy("Country_RegionIndex", "Province_StateIndex")
        .orderBy($"Date".asc)))
      .withColumn("Fatalities", sum("Fatalities").over(
        Window
          .partitionBy("Country_RegionIndex", "Province_StateIndex")
          .orderBy($"Date".asc))).cache()
    ...
    val finalDf = predictedDf.
      join(
        ...
        "left_outer"
      ).select(countryProvinceDf("Country_Region"), countryProvinceDf("Province_State"), predictedDf("Date"),  predictedDf("ConfirmedCases"), predictedDf("Fatalities")).cache()
