...
/** Main class */
object ForecastGBTMyCustomKaggleData {
...
With the rise in tempertaure, the confirmed cases tend to slow down (negative correlation).
 Hence, people in colder environments and cooler climatic conditions are much prone to the transmission of COVID-19.
  ...
  def learnModelForFeature(groupedDf:DataFrame, featureName:String): Unit = {
    val adjustedGroupedDf = addTimeValColumns(groupedDf.select("Country_RegionIndex", "Province_StateIndex",  "Date",  featureName).cache()
      , "Date", featureName, "Country_RegionIndex", "Province_StateIndex", "Date")
      .na.fill(0.0)
...
*/
    ...
    val readyDf = //new VectorAssembler().setInputCols(Array("CountryIndex", "Date", "Recovered_Cases","Dead_Cases"))
      new VectorAssembler().setInputCols("Country_RegionIndex"+:"Province_StateIndex"+:adjustedGroupedDf.columns.filter(_.startsWith(featureName+"_")))
    ...
    val  paramGrid = new ParamGridBuilder()
      ...
      .addGrid(gbt.maxDepth, Array(10, 15, 20)) //best 10
      .build()
...
*/
    model.save(s"models/KaggleDataModel_v3/GBTRegressionModel_total_cov_$featureName.ml")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val groupedDf =  spark.read.format("csv")
      .option("header","false")
      .option("inferSchema", "true")
      .load("kaggle_data/in/fullInputDf.csv")
